...
Therefore, a common use case for the UML Enumeration is to be used as a reference to an externally defined controlled list (see xref:../transformation/transf-rules3.adoc#sec:enumeration[enumerations]). 

Another possible use of UML Enumeration is to define a controlled list within the conceptual model.
In that setup, a UML Enumeration element contains one or more enumeration items. 
This approach is not recommended as it violates separation of concerns principle and hinders independent development and management of the controlled vocabulary.
