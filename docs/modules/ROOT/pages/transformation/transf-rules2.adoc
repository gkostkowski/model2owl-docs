...
Within the SHACL PropertyShape corresponding to a dependency relation linked to a given source UML Class, specify a NodeShape constraining the range of the dependency. Refer to a node shape for an enumeration item as defined in xref:transf-rules3.adoc#rule:enumeration-item-ds[transformation rule for enumeration item].
====
