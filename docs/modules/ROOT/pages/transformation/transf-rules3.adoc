...
|Enumeration |<<rule:enumeration-core>> | <<rule:enumeration-ds>> |<<rule:enumeration-rc>>
...
[#rule:enumeration-ds,source,XML,caption='',title='{example-caption} {counter:rule-cnt:D01}. Enumeration  -- in data shape layer',reftext='{example-caption} {rule-cnt}']
====
Specify SHACL NodeShape declaration axiom for each UML Enumeration. The URIs of the node shape is deterministically generated from the UML Enumeration name. Set `skos:ConceptScheme` as the expected type within the node shape.
====

[cols="a,a", options="noheader"]
|===
|
.shape in Turtle syntax
[source,Turtle]
----
...
shape:EnumerationName a sh:NodeShape ;
	sh:targetClass skos:ConceptScheme .
----
|
.shape in RDF/XML syntax
[source,XML]
----
<rdf:Description rdf:about="http://base.shape.uri/EnumerationName">
...
====
Specify SHACL NodeShape for items of an enumeration for each UML Enumeration. The URIs of the node shape is deterministically generated from the UML Enumeration name. Apply one of the two possible node shapes, depending the _constraint level_ set for the UML Enumeration as defined in xref:../uml/conv-enumerations.adoc#rule:enumeration-constraint-level[rule:enumeration-constraint-level]:

. _permissive_: set `skos:Concept` as the expected type of the enumeration item
. _restrictive_: set constraint for the enumeration item belonging to a concept scheme represented by the enumeration URI
====
...
shape:EnumerationName-itemShape a sh:NodeShape ;
...
[source,XML]
----
<rdf:Description rdf:about="http://base.shape.uri/EnumerationName-itemShape">
  <rdf:type rdf:resource="http://www.w3.org/ns/shacl#NodeShape"/>
  <sh:targetClass rdf:resource="http://www.w3.org/2004/02/skos/core#Concept"/>
</rdf:Description>
----
|
...
<rdf:Description rdf:about="http://base.shape.uri/EnumerationName-itemShape">
  ...
  <sh:property rdf:parseType="Resource">
    ...
    <sh:hasValue rdf:resource="http://base.onto.uri/EnumerationName"/>
  </sh:property>
</rdf:Description>
----
